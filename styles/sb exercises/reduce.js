...
function extractValue(arr, key) {
    const result = arr.reduce(function(name,nextWord){
       name.push(nextWord[key]);
       return name;
    },[]);
    return result;
}
...
let vowels = 'aeiou';
let results= str.split('').reduce(function(accumulator,next){
    if(vowels.indexOf(next.toLowerCase()) !== -1){
    if(accumulator[next.toLowerCase()]){
accumulator[next.toLowerCase()]++;
 }else{
    accumulator[next.toLowerCase()] = 1;
}
}
return accumulator;
},{});
...
function addKeyAndValue(arr, key, value) {
    results = arr.reduce(function(accumulator,next,ind){
        accumulator[ind][key]= value;
        return accumulator;
    },arr);
    return results;
}
...
function partition(arr, callback) {
    results = arr.reduce(function(accumulator,next){
